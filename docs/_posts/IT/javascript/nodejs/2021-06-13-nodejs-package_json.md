...
```
 ...
 - foo라는 패키지가 있다면 이 패키지를 설치한 뒤, require('foo');를 실행했을때 main으로 지정한 모듈의 exports 객체가 반환됩니다.
...
package.json에는 버전 정보를 저장할 때 `version range`를 사용합니다.

version range는 특정 버전이 아닌 버전의 _범위_ 를 의미합니다.

간혹 npm install을 진행하더라도 다른 node_modules가 생성되는 경우가 있습니다.

> 1) npm 버전이 다른 경우     
2) version range에 제대로 버전을 명시하지 않은 경우      
3) 패키지가 새로운 버전이 배포된 경우

package-lock.json 은 package.json이 수정되거나 node_modules의 구조가 변경될 때 당시의 의존성에 대한 정보를 가지고 자동 생성됩니다.

또, package-lock.json이 존재하는 경우 package.json을 이용하지 않고 node_modules를 구성합니다.

결국 동일한 node_modules를 생성하게 하여 같은 의존성을 보장하는 파일입니다.

### npm ci 커맨드
```shell
    $ npm ci
```
 - npm install 명령어는 package.json의 dependencies와 devDependencies를 기준으로 모듈을 설치합니다.
 - npm ci 명령어는 package.json보다 package-lock.json을 기준으로 설치합니다.
 - 두 파일의 모듈 버전이 다른 경우 package-lock.json을 기준으로 package.json을 수정하고 명시되지 않은 부분에서 오류를 발생시키기 때문에 Applcation 관리의 안정성이 높습니다.
